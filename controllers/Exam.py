...
class ExamController:
    ...
    def handle_edit_exam(self, edit_exam_data):
        ...
        return {"message": "Exam edited successfully", "status": status.HTTP_200_OK}

    def handle_publish_exam(self, publish_info):
        self.service.publish_exam(publish_info)
        return {"message": "Exam published successfully", "status": status.HTTP_200_OK}
    ...
    def handle_get_resolutions(self, course_id, user_id, status):
        resolutions = self.service.get_resolutions(course_id, user_id, status)
        return self._get_list_response(resolutions)

    def handle_get_resolution(self, course_id, student_id, user_id, name):
        exam_info = self.service.get_resolution(course_id, name, student_id, user_id)
    ...
    def handle_grade_resolution(self, course_id, grade_resolution_data):
        ...
        return {
            "message": "Resolution graded successfully",
    ...
    def handle_resolve_exam(self, course_id, answers):
        ...
        return {"message": "Exam completed successfully", "status": status.HTTP_200_OK}
